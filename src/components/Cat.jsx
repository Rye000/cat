...
2025-02-15,6`;
    ...
    const calculateDiet = () => {
        ...
        if (calcMode === "auto") {
            const currentIntake =
                basicCanKcal + basicSnackKcal + customStapleKcal + customWetKcal + customSnackKcal + customOtherKcal;
            ...
            totalUsed =
                basicCanKcal + basicSnackKcal + ziwiKcal + customStapleKcal + customWetKcal + customSnackKcal + customOtherKcal;
                ...
                <div className="flex bg-gray-100 p-1 rounded-xl">
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-xs font-bold transition-all ${
                            activeTab === "daily" ? "bg-white text-orange-600 shadow-sm" : "text-gray-400"
                        }`}>
                        <Utensils size={16} /> 記錄
                    </button>
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-xs font-bold transition-all ${
                            activeTab === "tools" ? "bg-white text-blue-600 shadow-sm" : "text-gray-400"
                        }`}>
                        <Activity size={16} /> 計算
                    </button>
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-xs font-bold transition-all ${
                            activeTab === "weight" ? "bg-white text-purple-600 shadow-sm" : "text-gray-400"
                        }`}>
            ...
            <div className="flex-1 p-4 overflow-y-auto pb-20">
                ...
                {activeTab === "daily" && (
                    <div className="space-y-4 animate-fade-in">
                            ...
                            {calcMode === "auto" ? (
                                <div className="text-center py-2">
                                    ...
                                    <div className="text-5xl font-black text-orange-700 tracking-tighter">
                                        {formatNum(results.ziwiGrams)} <span className="text-lg">g</span>
                                    </div>
                            ...
                            <div
                                ...
                                <div>
                                    ...
                                    <div className="text-2xl font-black">
                                        {formatNum(results.totalUsed)} <span className="text-sm font-normal">kcal</span>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="text-xs opacity-70 mb-1">{results.isOver ? "已超標" : "距離目標"}</p>
                                    <div className="text-xl font-bold flex items-center gap-1 justify-end">
                                        {results.isOver ? <AlertCircle size={18} /> : <CheckCircle2 size={18} />}
                                ...
                                <div className="space-y-2">
                                    {[can1Id, can2Id].map((currentId, idx) => (
                                        <select
                                            key={idx}
                                            value={currentId}
                                            onChange={(e) => (idx === 0 ? setCan1Id(e.target.value) : setCan2Id(e.target.value))}
                                            className="w-full p-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-blue-300">
                            ...
                            <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
                                <div className="flex justify-between items-center mb-2">
                                    <h3 className="font-bold text-gray-700 text-sm flex items-center gap-2">
                                        <div className="w-1 h-4 bg-yellow-400 rounded-full"></div> 衛仕爆爆桶 (
                                        {snackGrams * SNACK_KCAL_PER_G} kcal)
                                    </h3>
                        ...
                        <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
                            <h3 className="font-bold text-gray-700 text-sm mb-3 flex items-center gap-2">
                                <div className="w-1 h-4 bg-purple-400 rounded-full"></div> 自訂 / 額外攝取
                            </h3>
                            <div className="flex flex-col gap-2 mb-3 bg-gray-50 p-2 rounded-lg">
                                <div className="flex gap-2">
                                    <input
                                        ...
                                        className="flex-1 p-2 text-sm bg-white border border-gray-200 rounded focus:outline-none focus:border-purple-300"
                                    />
                                    <input
                                        type="text" // 改成 text
                                        inputMode="numeric" // 手機上還是會跳數字鍵盤
                                ...
                                <div className="flex gap-2">
                                    <select
                                        ...
                                        className="flex-1 p-2 text-sm bg-white border border-gray-200 rounded text-gray-600 focus:outline-none">
                                    ...
                                    {customFoods.map((item) => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <span className="font-mono text-gray-500">{item.kcal}</span>
                                                    <button
                        ...
                        <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100 mb-6">
                            ...
                            <div className="h-4 w-full bg-gray-100 rounded-full flex overflow-hidden mb-3">
                                <div
                                    style={{ width: `${(results.distribution.staple / targetKcal) * 100}%` }}
                                    className="bg-green-500 h-full"></div>
                                <div
                                    style={{ width: `${(results.distribution.wet / targetKcal) * 100}%` }}
                                    className="bg-blue-400 h-full"></div>
                                <div
                                    style={{ width: `${(results.distribution.snack / targetKcal) * 100}%` }}
                                    className="bg-yellow-400 h-full"></div>
                                <div
                                    style={{ width: `${(results.distribution.other / targetKcal) * 100}%` }}
                                    className="bg-purple-400 h-full"></div>
                            </div>
                            <div className="grid grid-cols-4 gap-1 text-center">
                                {[
                                    { label: "主食", val: results.distribution.staple, color: "text-green-600" },
                                    { label: "濕食", val: results.distribution.wet, color: "text-blue-600" },
                                    { label: "零食", val: results.distribution.snack, color: "text-yellow-600" },
                                    { label: "其他", val: results.distribution.other, color: "text-purple-600" },
                                ].map((item, idx) => (
                                    <div key={idx} className="flex flex-col">
                                        <span className="text-[10px] text-gray-400">{item.label}</span>
                                        <span className={`text-xs font-bold ${item.color}`}>{Math.round(item.val)}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                                ...
                                {FACTOR_DATA.map((factor) => (
                                    <div
                                        ...
                                        <div className="flex justify-between items-center mb-1">
                                            <span
                                                className={`font-bold ${
                                                    selectedFactorId === factor.id ? "text-blue-700" : "text-gray-700"
                                                }`}>
                                                {factor.label}
                                            </span>
                            ...
                            <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl shadow-lg p-6 text-white">
                                <div className="mb-4">
                                    <div className="flex justify-between items-end mb-2">
                                        <label className="text-sm font-bold text-gray-300">第三步：決定具體係數</label>
                                        <span className="text-2xl font-bold text-yellow-400">{formatNum(specificFactor)}</span>
                                    </div>
                                    <input
                                        type="range"
                                        min={FACTOR_DATA.find((f) => f.id === selectedFactorId).range[0]}
                                        max={FACTOR_DATA.find((f) => f.id === selectedFactorId).range[1]}
                                        step="0.1"
                                        value={specificFactor}
                                        onChange={(e) => setSpecificFactor(parseFloat(e.target.value))}
                    ...
                    <div className="space-y-4 animate-fade-in">
                            ...
                            <div className="h-[250px] w-full">
                                {isLoadingWeight ? (
                                    ...
                                    <ResponsiveContainer width="100%" height="100%">
                                        <LineChart data={filteredWeightData} margin={{ top: 5, right: 20, left: -20, bottom: 5 }}>
                        ...
                        <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
                            ...
                            <div className="divide-y divide-gray-100 max-h-[300px] overflow-y-auto">
                                ...
                                {filteredWeightData.length === 0 && (
                                    <div className="p-4 text-center text-xs text-gray-400">沒有符合篩選條件的記錄</div>
                                )}
                            </div>
                        </div>
                    </div>
