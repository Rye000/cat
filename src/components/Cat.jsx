...
const CatDietApp = () => {
    ...
    const handleLocalUpdate = (newDate, newWeight, cat = "cat1") => {
        ...
        const newRecord = {
            ...
            date: newDate.slice(5).replace("-", "/"), // 格式化成 MM/DD
            weight: parseFloat(newWeight),
            year: parseInt(newDate.split("-")[0]),
            month: parseInt(newDate.split("-")[1]),
        };

        setWeightHistory((prev) => [...prev, newRecord].sort((a, b) => new Date(a.fullDate) - new Date(b.fullDate)));
...
2025-02-15,6`;
    ...
    const applyDerToTarget = () => {
        ...
        Swal.fire({
            icon: "success",
            title: "成功",
            text: `已更新目標熱量為 ${Math.round(der)} kcal`,
                    ...
                    <div className="space-y-4 animate-fade-in">
                            ...
                            <div
                                ...
                                <div>
                                    ...
                                    <div className="text-2xl font-black">
                                        {formatNum(results.totalUsed)} <span className="text-base font-normal">kcal</span>
                                    </div>
                                </div>
                            ...
                            <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
                                ...
                                <div className="space-y-3">
                                    ...
                                    {[can1Id, can2Id].map((currentId, idx) => (
                                        <CustomFormControl fullWidth size="small" key={idx}>
                                            <InputLabel id={`can-select-label-${idx}`}>
                                                {idx === 0 ? "早上" : "晚上"}罐罐
                                            </InputLabel>
                                            <Select
                                                className="mb-3"
                                                labelId={`can-select-label-${idx}`}
                                                value={currentId}
                                                label={`選擇罐頭 ${idx + 1}`} // label 屬性要跟 InputLabel 文字一致，框線缺口才會正確
                                                onChange={(e) =>
                                                    idx === 0 ? setCan1Id(e.target.value) : setCan2Id(e.target.value)
                                                }
                                                sx={{ backgroundColor: "#fff" }} // 確保背景是白的，蓋住後面的底色
                                            >
                                                {CAN_DATABASE.map((can) => (
                                                    <MenuItem key={can.id} value={can.id}>
                                                        {/* 這裡可以排版一下，讓熱量靠右或變色 */}
                                                        <div
                                                            style={{
                                                                display: "flex",
                        ...
                        <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
                            ...
                            <div className="flex flex-col gap-2 mb-3 bg-gray-50 p-2 rounded-lg">
                                ...
                                <div className="flex gap-2 items-stretch mt-2">
                                    {" "}
                                    {/* items-stretch 確保按鈕跟輸入框一樣高 */}
                                    <CustomFormControl size="small" className="flex-1">
                                    ...
                                    <button
                                        ...
                                        className="bg-purple-500 text-white px-4 rounded-md hover:bg-purple-600 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm transition-colors flex items-center justify-center"
                                        // 如果按鈕高度跑掉，可以強制指定 height: '40px' (對應 size="small" 的高度)
                                    >
                                        <Plus size={20} />
                                    </button>
                                </div>
                        ...
                        <div className="bg-white rounded-xl shadow-sm p-4 border border-gray-100 mb-6">
                            ...
                            <div className="h-4 w-full bg-gray-100 rounded-full flex overflow-hidden mb-3">
                                <div
                                    style={{
                                        width: `${(results.distribution.staple / targetKcal) * 100}%`,
                                    }}
                                ...
                                <div
                                    style={{
                                        width: `${(results.distribution.snack / targetKcal) * 100}%`,
                                    }}
                                    className="bg-yellow-400 h-full"></div>
                                <div
                                    style={{
                                        width: `${(results.distribution.other / targetKcal) * 100}%`,
                                    }}
                            ...
                            <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl shadow-lg p-6 text-white">
                                <div className="mb-4">
                                    <div className="flex justify-between items-end mb-2">
                                        <label className="text-base font-bold text-gray-300">第三步：決定具體係數</label>
                                ...
                                <div className="border-t border-gray-700 pt-4 text-center">
                                    <p className="text-gray-400 text-base mb-1">DER (每日能量需求)</p>
                            ...
                            <>
                                ...
                                <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
                                    <div className="p-3 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                                        <h4 className="font-bold text-gray-600 text-base">詳細記錄列表</h4>
                                    ...
                                    <div className="divide-y divide-gray-100 max-h-[300px] overflow-y-auto">
                                        {[...filteredWeightData].reverse().map((record, idx) => (
                                            <div
                                                ...
                                                <span className="text-base font-bold text-gray-800">{record.weight} kg</span>
                                            </div>
                                        ))}
                                        {filteredWeightData.length === 0 && (
